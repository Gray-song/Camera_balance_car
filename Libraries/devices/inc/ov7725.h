...
/* 定义引脚 */
#define BOARD_OV7725_I2C						I2C0_SCL_PB02_SDA_PB03
...
#define BOARD_OV7725_DMA      			HW_DMA_CH2

#define BOARD_OV7725_DATA_PORT			HW_GPIOC
#define BOARD_OV7725_DATA_OFFSET    (8)  //数据口8-15


#define IMAGE_SIZE  0    //分辨率
...
//int ov7725_set_image_size(ov7725_size size); //根据像素大小写寄存器  
void Image_Decompression(uint8_t *data1,uint8_t *data2); //数组解压函数，*data1为源数组地址，*data2为目标数组
int SCCB_Init(uint32_t I2C_MAP);
void OV_ISR(uint32_t index);
void write_reg(void );  //写寄存器
void SendImage_PC(uint32_t UART_instance , uint8_t *str); //向上位机发送数据
void _7725_DMA(void );  //初始化DMA
void GPIO_PORT(void ); //行场PCLK设置
void SXT_7725_Init(void); //初始化ov7725
//void Tu_Xiang(void);//图像显示
