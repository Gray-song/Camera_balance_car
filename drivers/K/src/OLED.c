...
#define LCD_SCL_CLR()	GPIO_ResetBit(HW_GPIOD,7)//SCL 引脚置 0
#define LCD_SCL_SET()	GPIO_SetBit(HW_GPIOD,7)//SCL 引脚置 1
#define LCD_SDA_CLR()	GPIO_ResetBit(HW_GPIOD,6)//SDA_DIN 引脚置 0
#define LCD_SDA_SET()	GPIO_SetBit(HW_GPIOD,6)//SDA_DIN 引脚置 1
#define LCD_RST_CLR()	GPIO_ResetBit(HW_GPIOD,5)////RST 引脚置 0
#define LCD_RST_SET()	GPIO_SetBit(HW_GPIOD,5)////RST 引脚置 1
#define LCD_DC_CLR()	GPIO_ResetBit(HW_GPIOD,4)////DC 引脚置 0
#define LCD_DC_SET()	GPIO_SetBit(HW_GPIOD,4)////DC 引脚置 1
...
void OLED_Init(void)        
{
  ...
  //DDRA=0XFF;
  //引脚初始化
//	GPIO_InitTypeDef  GPIOE_InitStruct11;
//	GPIOE_InitStruct11.pin=GPIO_Pin_23|GPIO_Pin_22|GPIO_Pin_21|GPIO_Pin_20;//
//	GPIOE_InitStruct11.Direction=GPIO_Out;//输出
//	GPIO_Init(PORTE,PTE, &GPIOE_InitStruct11);
...
{         	
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				OLED_ShowChar(x+(size2/2)*t,y,' ',12);
				continue;
			}else enshow=1; 
		 	 
		}
	 	OLED_ShowChar(x+(size2/2)*t,y,temp+'0',12); 
	}
} 
...
//	u8 AD_DATA;
	...
	for(i=0;i<600;i++){
		...
		count=i%10;
		if((count)<8){
			LCD_Put_Column(59-line,count*8,(*((DATA+i))));
		}else{
			LCD_Put_Column(119-line,(count-2)*8,(*(DATA+i)));
		}
	}
...
/************************************************___________________________
	OLED显示小钻风图像60*80												|														|
例：																						|				 										|
	show_60x80_7725( (u16*)& pixel[0]);				 		|		  											|										
	pixel为摄像头原始数据数组 一维数组						|		  											|
																								|														|
																								|___________________________|			 		
********************************************************************************/
extern void show_60x80_7725(u8* DATA){
	...
	for(i=0;i<600;i++){
		if(i<560){
				count=i%10;
				j=0;
				AD_DATA[7]=((*(DATA+i))&(128))>>7|(((*(DATA+i+10))&(128))>>6)|				//最高位 第八位
								(((*(DATA+i+20))&(128))>>5)|(((*(DATA+i+30))&(128))>>4)|																								
								(((*(DATA+i+40))&(128))>>3)|(((*(DATA+i+50))&(128))>>2)|																				
								(((*(DATA+i+60))&(128))>>1)|(((*(DATA+i+70))&(128))); 
				LCD_Put_Column(count*8+0,(i/80)*8,(AD_DATA[7]));
			
				j=1;
				AD_DATA[6]=(((*(DATA+i))&(128>>j))>>6)|(((*(DATA+i+10))&(128>>j))>>5)|		//第7位
								(((*(DATA+i+20))&(128>>j))>>4)|(((*(DATA+i+30))&(128>>j))>>3)|																								
								(((*(DATA+i+40))&(128>>j))>>2)|(((*(DATA+i+50))&(128>>j))>>1)|																				
								(((*(DATA+i+60))&(128>>j)))|(((*(DATA+i+70))&(128>>j))<<1); 
				LCD_Put_Column(count*8+j,(i/80)*8,(AD_DATA[6]));
			
				j=2;
				AD_DATA[5]=(((*(DATA+i))&(128>>j))>>5)|(((*(DATA+i+10))&(128>>j))>>4)|			//第六位
								(((*(DATA+i+20))&(128>>j))>>3)|(((*(DATA+i+30))&(128>>j))>>2)	 |																								
								(((*(DATA+i+40))&(128>>j))>>1)|(((*(DATA+i+50))&(128>>j)))|																				
								(((*(DATA+i+60))&(128>>j))<<1)|(((*(DATA+i+70))&(128>>j))<<2); 
				LCD_Put_Column(count*8+j,(i/80)*8,(AD_DATA[5]));
				
				j=3;
				AD_DATA[4]=(((*(DATA+i))&(128>>j))>>4)|(((*(DATA+i+10))&(128>>j))>>3)|		//第五位
								(((*(DATA+i+20))&(128>>j))>>2)|(((*(DATA+i+30))&(128>>j))>>1)|																								
								(((*(DATA+i+40))&(128>>j)))|(((*(DATA+i+50))&(128>>j))<<1)|																				
								(((*(DATA+i+60))&(128>>j))<<2)|(((*(DATA+i+70))&(128>>j))<<3); 
				LCD_Put_Column(count*8+j,(i/80)*8,(AD_DATA[4]));
				
				j=4;
				AD_DATA[3]=(((*(DATA+i))&(128>>j))>>3)|(((*(DATA+i+10))&(128>>j))>>2)|		//第4
								(((*(DATA+i+20))&(128>>j))>>1)|(((*(DATA+i+30))&(128>>j)))|																								
								(((*(DATA+i+40))&(128>>j))<<1)|(((*(DATA+i+50))&(128>>j))<<2)|																				
								(((*(DATA+i+60))&(128>>j))<<3)|(((*(DATA+i+70))&(128>>j))<<4); 
				LCD_Put_Column(count*8+j,(i/80)*8,(AD_DATA[3]));
				
				j=5;
				AD_DATA[2]=(((*(DATA+i))&(128>>j))>>2)|(((*(DATA+i+10))&(128>>j))>>1)|		//第3
								(((*(DATA+i+20))&(128>>j)))|(((*(DATA+i+30))&(128>>j))<<1)|																								
								(((*(DATA+i+40))&(128>>j))<<2)|(((*(DATA+i+50))&(128>>j))<<3)|																				
								(((*(DATA+i+60))&(128>>j))<<4)|(((*(DATA+i+70))&(128>>j))<<5); 
				LCD_Put_Column(count*8+j,(i/80)*8,(AD_DATA[2]));
			
				j=6;
				AD_DATA[1]=(((*(DATA+i))&(128>>j))>>1)|(((*(DATA+i+10))&(128>>j)))|		//第二
								(((*(DATA+i+20))&(128>>j))<<1)|(((*(DATA+i+30))&(128>>j))<<2)|																								
								(((*(DATA+i+40))&(128>>j))<<3)|(((*(DATA+i+50))&(128>>j))<<4)|																				
								(((*(DATA+i+60))&(128>>j))<<5)|(((*(DATA+i+70))&(128>>j))<<6); 
				LCD_Put_Column(count*8+j,(i/80)*8,(AD_DATA[1]));
				
				j=7;
				AD_DATA[0]=(((*(DATA+i))&(128>>j)))|(((*(DATA+i+10))&(128>>j))<<1)|	//第一位   数据最低位
								(((*(DATA+i+20))&(128>>j))<<2)|(((*(DATA+i+30))&(128>>j))<<3)|																								
								(((*(DATA+i+40))&(128>>j))<<4)|(((*(DATA+i+50))&(128>>j))<<5)|																				
								(((*(DATA+i+60))&(128>>j))<<6)|(((*(DATA+i+70))&(128>>j))<<7); 
				LCD_Put_Column(count*8+j,(i/80)*8,(AD_DATA[0]));
